...
INSET1=`libra query resource --resource-path-string 0x1::stake::ValidatorSet 0x1 | jq -r '.active_validators[].addr' | grep -P $accountlaststring`
sleep 1
if [[ -z $INSET1 ]]
then
  INSET1=0
...
then
  ...
  send_discord_message "$message"
  tmux send-keys -t node:0 "RUST_LOG=info libra node >> ~/logs/validator.log" C-m
...
sleep 0.2
EPOCHREWARD1=$(libra query resource --resource-path-string 0x1::proof_of_fee::ConsensusReward 0x1 | jq -r '.nominal_reward' | awk '{printf "%.2f\n", $1/1000000}')
sleep 0.2
NETREWARD1=$(libra query resource --resource-path-string 0x1::proof_of_fee::ConsensusReward 0x1 | jq -r '.net_reward' | awk '{printf "%.2f\n", $1/1000000}')
sleep 0.2
BALANCET1=$(libra query balance $accountinput | jq -r '.unlocked, .total' | paste -sd " / " | awk '{printf "%.2f %.2f", $1, $2}' | cut -d ' ' -f 2)
sleep 0.2
TBALANCET1=$(echo "$BALANCET1" | sed -E ':a;s/(.*[0-9])([0-9]{3})/\1,\2/;ta')
BALANCEU1=$(libra query balance $accountinput | jq -r '.unlocked, .total' | paste -sd " / " | awk '{printf "%.2f %.2f", $1, $2}' | cut -d ' ' -f 1)
sleep 0.2
...
while true; do
  ...
  accountlaststring="${accountinput: -6}"
  INSET2=`libra query resource --resource-path-string 0x1::stake::ValidatorSet 0x1 | jq -r '.active_validators[].addr' | grep -P $accountlaststring`
  sleep 1
  if [[ -z $INSET2 ]]
  then
    INSET2=0
  ...
  sleep 0.2
  JAIL=`libra query resource --resource-path-string 0x1::jail::Jail $accountinput | jq -r .is_jailed | grep -q "false" && echo "Not jailed." || echo "You are jailed."`
  sleep 0.2
  EPOCHREWARD2=$(libra query resource --resource-path-string 0x1::proof_of_fee::ConsensusReward 0x1 | jq -r '.nominal_reward' | awk '{printf "%.2f\n", $1/1000000}')
  sleep 0.2
  NETREWARD2=$(libra query resource --resource-path-string 0x1::proof_of_fee::ConsensusReward 0x1 | jq -r '.net_reward' | awk '{printf "%.2f\n", $1/1000000}')
  sleep 0.2
  BALANCET2=$(libra query balance $accountinput | jq -r '.unlocked, .total' | paste -sd " / " | awk '{printf "%.2f %.2f", $1, $2}' | cut -d ' ' -f 2)
  sleep 0.2
  TBALANCET2=$(echo "$BALANCET2" | sed -E ':a;s/(.*[0-9])([0-9]{3})/\1,\2/;ta')
  BALANCEU2=$(libra query balance $accountinput | jq -r '.unlocked, .total' | paste -sd " / " | awk '{printf "%.2f %.2f", $1, $2}' | cut -d ' ' -f 1)
  sleep 0.2
  TBALANCEU2=$(echo "$BALANCEU2" | sed -E ':a;s/(.*[0-9])([0-9]{3})/\1,\2/;ta')
  VOUCH=`libra query resource --resource-path-string 0x1::vouch::MyVouches $accountinput | jq '.my_buddies | map(select(startswith("0x"))) | length'`
  sleep 0.2
