...
do
    ...
    then
        export TIME=`date +%Y-%m-%dT%I:%M:%S`
        syn1=`curl 127.0.0.1:9101/metrics 2> /dev/null | grep diem_state_sync_version`
        if [ -z $syn1 ]
        then
        ...
        syn1=`curl 127.0.0.1:9101/metrics 2> /dev/null | grep diem_state_sync_version`
        sync1=`echo "$syn1"|sed -n -e '3p'`
        syn20=`echo $sync1 | grep -o '[0-9]*'`
        ...
        then
            ...
            export syn2=`curl 127.0.0.1:9101/metrics 2> /dev/null | grep diem_state_sync_version`
            export sync2=`echo "$syn2"|sed -n -e '3p'`
