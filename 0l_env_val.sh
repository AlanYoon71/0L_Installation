...
sleep 2
echo ""
echo "Script starts! Maybe it'll be over in three hours.^^;"
sleep 1
echo ""
echo ""
...
echo ""
sleep 2
sudo apt install -y sudo curl git vim sysstat zip unzip daemontools bc jq build-essential cmake clang llvm libgmp-dev secure-delete pkg-config libssl-dev lld tmux
echo ""
...
sleep 2
sudo \cp -f ./0l_tmux_val.sh /home/node &> /dev/null ; sudo \cp -f ./autopay_batch.json /home/node &> /dev/null
sleep 2
sudo chmod +x /home/node/0l_tmux_val.sh
sudo chmod go+rw /home/node/0l_tmux_val.sh
sudo -u node /home/node/0l_tmux_val.sh &&
CUR_DATE=`date +%Y%m%d_%I%M%S_%p` &&
